...
| 组件 | 安装位置 | 作用 |
|------|----------|------|
| **shipfast CLI** | Windows / Linux / macOS | 部署指令、服务器管理、**内置 Web GUI + Cloud** |
| **shipfast-agent** | Linux VPS | 构建镜像、运行容器、Nginx、HTTPS |
| **shipfast-cloud** | **内嵌在 CLI 中**（`shipfast web` 自动启动） | 账号、License、运营后台 `/admin` |
| **shipfast-discovery** | 可选部署 | 服务发现（Git 模式免费，无需部署） |
...
| `ShipFast-Discovery-<版本>-*.tar.gz` / `.zip` | 服务发现（可选） |
| `SHA256SUMS` | 校验清单 |
...
- 仪表板、部署向导、运维、密钥、模板市场；管理后台 `/admin`
...
| Cloud（内嵌） | `shipfast web` 自动启动（端口 19000） |
...
|-----------|-------|------|
| **shipfast CLI** | Windows / Linux / macOS | Deploy, server management, **embedded Web GUI + Cloud** |
| **shipfast-agent** | Linux VPS | Build, containers, Nginx, HTTPS |
| **shipfast-cloud** | **Embedded in CLI** (auto-started with `shipfast web`) | Account, license, `/admin` console |
| **shipfast-discovery** | Optional deploy | Service discovery (Git mode free, no deploy needed) |
...
| `ShipFast-Discovery-<ver>-*.tar.gz` / `.zip` | Service discovery (optional) |
...
- Dashboard, deploy wizard, ops, secrets, marketplace; admin console at `/admin`
...
| Cloud (embedded) | Auto-started with `shipfast web` (port 19000) |
